...
class UploadsController extends AppController {
	...
	public function beforeFilter() {
        ...
        $this->Auth->allow('index','view','download','upload','display');
    }
	...
	function index($idloai=null){		
		...
		for ($i=0;$i<count($files);$i++){
			  $countfile=$this->Upload->find('count',array('conditions'=>array('Upload.idloai'=>$files[$i]['Tblloaitailieu']['idloai'])));			
				array_push($files[$i],array("countfile"=>$countfile));
		}
...
//		mysql_query($sql1);
	...
	function populateEditfileUpload($idloai=NULL,$page=null,$end=null){
		...
		$data=$this->Upload->find('all',array('conditions'=>array('Upload.idloai'=>$idloai), 'limit' => $this->numberRecord, 'offset'=>($page-1)*$this->numberRecord));
		$this->set("datas",$data);
	...
	public function display($idUpload,$page=null,$end=null) {
		$this->Auth->allow();
		
		$Upload=$this->Upload->find("first",array('conditions' => array('Upload.id' => $idUpload)));
		$tblloaitailieu = $this->Upload->Tblloaitailieu->read(null,$Upload['Upload']['idloai']);
		$this->set('tblloaitailieu',$tblloaitailieu);
		$get_url = "http://localhost/luatvnam/app/webroot/img/uploads"."/".$Upload['Upload']['name'];
		$this->set("url",$get_url);
		$this->set('Upload',$Upload);
		$data=$this->Upload->find('all',array('conditions'=>array('Upload.idloai'=>$Upload['Upload']['idloai'],'id <>'=>$idUpload), 'limit' => $this->numberRecord, 'offset'=>($page-1)*$this->numberRecord));
		$this->set("datas",$data);
		$this->populateEditfileUpload($Upload['Upload']['idloai'],$page,$end);
	}
